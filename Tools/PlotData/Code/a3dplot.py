...
def GetHCDATA(db, case, simulation, measurement):
    command = f"""
...
FROM (SELECT * FROM VoltageMeasurements VM WHERE Elemento like 'load.fakeload_bus_33998182_001' AND VM.[Case] = {case} AND VM.Simulation = {simulation} AND VM.Measurement = '{measurement}') AS _001
...
db = 'DB_Rede_3'
df = GetHCDATA(db, 1, 1, ' V1')

# Extract the values from the DataFrame
x = df.index  # Assuming the index represents the 96 steps during the day
y = df['Y']
z = df.columns  # Assuming the columns are ['Bus1', 'Bus2', 'Bus3']

# Reshape the data for plotting
x, z = zip(*[(xi, zi) for xi in x for zi in z])
y = y.values.flatten()
