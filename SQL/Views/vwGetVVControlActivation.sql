---------------------------------------------------------
-- This view returns all the cases that a VV control
...
CREATE OR ALTER VIEW vwGetVVControlActivation
AS
	-- Caso para identificar o VV atuando só
	WITH 
	voltavar AS (
		SELECT 
				*--DISTINCT(id_Summary)
	...
	voltawatt AS (
		...
		FROM spSummary_InvControlData 
		WHERE 
			( Measurement like 'volt-watt' AND [Value] = 9999 )
	)
